<template>
  <div>
    <div class="d-flex justify-content-center" v-if="jornada && !ausenteHoy">
    ...
    <div
      class="alert alert-dark d-flex w-auto mx-auto alert-dismissible fade show justify-content-center"
      v-if="!jornada"
    >
      <svg xmlns="http://www.w3.org/2000/svg" style="display: none">
        <symbol id="info-fill" fill="currentColor" viewBox="0 0 16 16">
      ...
      <span>
        <strong>Hoy no tiene jornada laboral</strong>
      </span>
      <button
        type="button"
        class="btn-close"
...
export default {
  ...
  methods: {
    ...
    async getAusenciaHoy() {
      try {
        const response = await axios.get(
          `/empresas/${this.user.empresas_id}/empleados/${this.user.id}/ausencias/info`
        );
        if (response.status === 204) {
          this.ausenteHoy = false;
        }
        this.ausenteHoy = response.data;
      } catch (error) {
        if (error.response && error.response.status === 403) {
          // Recargar la página para mostrar el formulario de inicio de sesión
          location.reload();
