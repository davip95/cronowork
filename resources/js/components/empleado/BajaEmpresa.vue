          ...
          <div class="card-body">
            <div class="row mb-3">
              <div class="alert alert-danger d-inline-block w-auto mx-auto">
                <span
                  ><strong>¡Cuidado {{ empleado.name }}!</strong> Vas a darte de
                  baja de tu empresa. Serás redirigido al login y tu cuenta se
                  mantendrá como usuario.</span
                >
              </div>
            </div>
            <div class="row">
  ...
  methods: {
    async bajaEmpleado() {
      try {
        this.$Progress.start();
        await axios.put(
          `/empresas/${this.empleado.empresas_id}/empleados/${this.empleado.id}/baja`
        );
